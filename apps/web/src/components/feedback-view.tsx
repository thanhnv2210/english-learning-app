...
export function FeedbackView({ feedback }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Overall band */}
      <div className={`rounded-xl p-4 ${needsWork ? 'bg-amber-50 border border-amber-200' : 'bg-green-50 border border-green-200'}`}>
        <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Overall Band</p>
        <div className="mt-1 flex items-end gap-2">
          <span className="text-3xl font-bold text-gray-900">{feedback.overallBand}</span>

  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {hasOutline && (
        <section>
          <Label>Outline</Label>
          <div className="mt-1 flex flex-col gap-2 rounded-xl border border-border bg-muted p-4">
            {[
          ...
          {outlineCritique && (
            <details className="mt-2 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 px-4 py-3 text-sm">
              ...
              <p className="mt-2 whitespace-pre-wrap leading-relaxed text-foreground">{outlineCritique}</p>
            </details>
          )}
        </section>
      )}
      ...
      <section>
        ...
        <div className="mt-1 rounded-xl border border-border bg-card p-4">
          <p className="whitespace-pre-wrap text-sm leading-relaxed text-foreground">{essay}</p>
        </div>
      </section>
    </div>
  )
...
function ExamCard({
  ...
  return (
    <div className={`rounded-xl border border-border bg-card p-5 transition-opacity ${isPending ? 'opacity-60' : ''}`}>
      {/* Header */}
      ...
      <div className="mt-3 flex flex-col gap-1.5">
        {preview.map((m) => (
          <p key={m.id} className="text-xs leading-relaxed text-muted-foreground">
            <span className="font-semibold capitalize text-faint">{m.role === 'assistant' ? 'Examiner' : 'You'}: </span>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        {exam.examTags.map(({ tag }) => (
          <span
            key={tag.id}
            className="flex items-center gap-1 rounded-full bg-subtle px-2.5 py-0.5 text-xs text-muted-foreground"
          >
            {tag.name}
            <button
        ...
        <form onSubmit={handleAddTag}>
          <input
            ...
            placeholder="+ tag"
            className="w-20 rounded-full border border-dashed border-border bg-transparent text-foreground px-2.5 py-0.5 text-xs outline-none transition-all focus:w-28 focus:border-blue-400 dark:focus:border-blue-500"
          />
        </form>
      </div>
        ...
        <button
          ...
          className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
            exam.feedback
              ...
              : 'border border-border text-faint hover:bg-muted'
          }`}
        >

...
export function TicketForm({ projectId, sprintId, ticket, isTemplate, onClose }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      if (ticket) {
        await updateTicketAction(ticket.id, { title: title.trim(), description: description.trim(), priority: priority as never, type: type as never })
      } else {
        await createTicketAction({ projectId, sprintId, title: title.trim(), description: description.trim(), priority: priority as never, type: type as never, isTemplate })
      }
      onClose()
    })
  }

...
export async function saveIdiom(data: {
  ...
  const [row] = await db
    .insert(idiomEntries)
    .values({ ...data, idiom: data.idiom.toLowerCase() })
    .onConflictDoNothing()
    .returning()
  return (row as IdiomCard) ?? null
}

export async function getAllIdioms(): Promise<IdiomCard[]> {
  return db
    .select()
    .from(idiomEntries)
...
export async function deleteIdiom(id: number): Promise<void> {
  await db.delete(idiomEntries).where(
    and(eq(idiomEntries.id, id), eq(idiomEntries.isSystem, false))
  )
}
 ...
 */
export async function getIdiomPracticeItems(): Promise<PracticeItem[]> {
  const rows = await getAllIdioms()

...
const ROLE_STYLES: Record<QuestionRole, { bg: string; border: string; text: string; badge: string }> = {
  ...
  'time':          { bg: 'bg-subtle',   border: 'border-border',   text: 'text-muted-foreground',   badge: 'bg-gray-500'   },
}
...
export function QuestionAnatomyGuide({ sections }: { sections: SkillSection[] }) {
  ...
  return (
    <section className="space-y-5">
      <h2 className="text-base font-bold text-foreground">Question Breakdowns by Skill</h2>

      {/* Skill tabs */}
      <div className="flex gap-1 border-b border-border">
        {sections.map((s) => {
          const isActive = s.skill === activeSkill
          const accent = SKILL_ACCENT[s.skill] ?? { active: 'border-muted-foreground text-muted-foreground bg-muted', tab: '' }
          return (
            <button
              key={s.skill}
              ...
              className={`flex items-center gap-1.5 border-b-2 px-4 py-2.5 text-sm font-medium transition-colors ${
                isActive
                  ...
                  : `border-transparent text-muted-foreground ${accent.tab}`
              }`}
            >
              <span>{s.icon}</span>
              {s.skill}
...
function QuestionCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      <button
        onClick={onToggle}
        className="flex w-full items-start gap-3 px-5 py-4 text-left hover:bg-muted"
      >
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-foreground text-xs font-bold text-background mt-0.5">
          {idx + 1}
        </span>
        ...
        <span className={`shrink-0 text-xs text-faint mt-1 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}>
          ▼
        </span>
      </button>
        ...
        <div className="border-t border-border px-5 pb-5 pt-4 space-y-4">
            ...
            {ex.fragments.map((f, fi) => {
              ...
              return (
                <div key={fi} className={`flex gap-3 rounded-lg border p-3 ${s.bg} ${s.border}`}>
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">{f.instruction}</p>
                </div>
              )
            })}
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-border bg-muted px-3 py-2.5">
              ...
              <p className="text-xs font-medium text-muted-foreground">{ex.answerType}</p>
            </div>
